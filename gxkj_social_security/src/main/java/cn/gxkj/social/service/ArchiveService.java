...
import cn.gxkj.model.social_security.Archive;
import cn.gxkj.model.social_security.ArchiveDetail;
import cn.gxkj.model.social_security.CityPaymentItem;
import cn.gxkj.model.social_security.UserSocialSecurity;
import cn.gxkj.social.client.UserCompanyPersonalFeignClient;
import cn.gxkj.social.dao.ArchiveDao;
import cn.gxkj.social.dao.ArchiveDetailDao;
...
public class ArchiveService {
    ...
    @Autowired
    private UserCompanyPersonalFeignClient userCompanyPersonalFeignClient;
    ...
    public List<ArchiveDetail> getReports(String yearMonth, String companyId) {
        //查询用户的社保列表（用户和基本社保数据）
        Page<Map> userSocialSecurityDaoPage = userSocialSecurityDao.findPage(companyId, null);
        List<ArchiveDetail> list = new ArrayList<>();
        for (Map map : userSocialSecurityDaoPage) {
            ...
            //获取个人信息
            Result personalInfo = userCompanyPersonalFeignClient.findPersonalInfo(userId);
            if (personalInfo.isSuccess()) {
//				vo.setUserCompanyPersonal(personalInfo.getData());
                //解析json对象数据
                UserCompanyPersonal userCompanyPersonal = JSON.parseObject(JSON.toJSONString(personalInfo.getData()), UserCompanyPersonal.class);
    ...
    private void getOtherData(ArchiveDetail vo, String yearMonth) {
        ...
        //查询当前对象的对应城市的保险
        List<CityPaymentItem> allByCityId = paymentItemService.findAllByCityId(userSocialSecurity.getProvidentFundCityId());
        for (CityPaymentItem cityPaymentItem : allByCityId) {
            //判断企业是否缴税
            if (cityPaymentItem.getSwitchCompany()) {
                ...
                BigDecimal divide = augend.divide(new BigDecimal(100));
                socialSecurityCompanyPay = socialSecurityCompanyPay.add(divide);
            }
            //个人
            if (cityPaymentItem.getSwitchPersonal()) {
                ...
                BigDecimal divide = augend.divide(new BigDecimal(100));
                socialSecurityPersonalPay = socialSecurityPersonalPay.add(divide);
            }
        }
